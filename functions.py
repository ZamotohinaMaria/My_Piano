...
def get_sounds(white_notes, black_flats):
    white_sounds = []
...
def draw_piano(active_whites, active_blacks, screen, HEIGHT, WIDTH, track, sec, mins, sample, i_note, view):
    ...
    if view == True:
        if sample[i_note] in white_notes_label:
            index = white_notes_label.index(sample[i_note])
            print(index)
            pygame.draw.rect(screen, 'gray', [10 + index * lenght_key, HEIGHT - height_key, lenght_key, height_key], 0, 2)
    ...
    while i < len_white and len_white > 0:
        if active_whites[i][1] == 0:
            if active_whites[i][3] == 1:
                track.append(mido.Message('note_off', note=active_whites[i][2], velocity=100, time=round((sec + time.time() % 1 + mins * 60) * 50)))
            active_whites.pop(i)
            len_white -= 1
    ...
    for i in range(len(black_flats_label)):
        ...
        if view == True:
            if sample[i_note] in black_flats_label:
                if black_flats_label.index(sample[i_note]) == i:
                    pygame.draw.rect(screen, 'gray', [10 + lenght_key*(1/1.5 + i + skip_count), HEIGHT - height_key, lenght_key/1.4, height_key*2/3], 0, 2)
        q = 0
            ...
            if active_blacks[q][0] == i:
                if active_blacks[q][1] == 0: 
                    if active_blacks[q][3] == 1:
                        track.append(mido.Message('note_off', note=active_blacks[q][2], velocity=100, time=round((sec + time.time() % 1 + mins * 60) * 50)))
                    active_blacks.pop(q)
                    len_black -= 1
    ...
    #     if sample[i_note] in black_flats_label:
    #         index = black_flats_label.index(sample[i_note])
...
def draw_keyboard(active_white, active_black, screen, HEIGHT, WIDTH, view):
...
def view_sample(screen, WIDTH, HEIGHT, view, key_learn):
    if view == True:
        if key_learn == None:
            ...
            sample_img =  pygame.image.load(learn_samples[key_learn])
            sample_img = pygame.transform.scale(sample_img, (WIDTH*(9/40), HEIGHT*(25/40)))
            screen.blit(sample_img, (WIDTH*(8/20), HEIGHT/15 + HEIGHT*(1/40) + 5, ))
...
def draw_sample(screen, HEIGHT, WIDTH, sample, i_note, view):
    lenght_key = (WIDTH - 20)/29
    height_key = HEIGHT/4
